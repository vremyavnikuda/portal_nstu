        ...
        <div class="form-container mat-elevation-z5" autocomplete="off">
            ...
            <form [formGroup]="form" (ngSubmit)="submitRegistration()">
                ...
                <mat-form-field class=" full-width">
                    ...
                    <input matInput formControlName="login" type="login" class="form-control" placeholder="Логин"
                           required autocomplete="off">
                </mat-form-field>

                <mat-form-field class="full-width">
                    <mat-label class="type-font-form">Имя</mat-label>
                    <input matInput formControlName="first_name" type="first_name" class="form-control"
                           placeholder="Имя" required autocomplete="off">
                </mat-form-field>

                <mat-form-field class="full-width">
                    <mat-label class="type-font-form">Фамилия</mat-label>
                    <input matInput formControlName="last_name" type="last_name" class="form-control"
                           placeholder="Фамилия" required autocomplete="off">
                </mat-form-field>

                <mat-form-field class="full-width">
                    <mat-label class="type-font-form">Отчество</mat-label>
                    <input matInput formControlName="middle_name" type="middle_name" class="form-control"
                           placeholder="Отчество" required autocomplete="off">
                </mat-form-field>
                ...
                <div>
                    ...
                    <mat-form-field>
                        ...
                        <input matInput formControlName="conf_password" [type]="hideConf ? 'conf_password':'text'"
                               placeholder="Подтверждение пароля" required autocomplete="off">
                        <button mat-icon-button matSuffix (click)="hideConf =!hideConf"
                                [attr.aria-label]="'Hide password'"
                                [attr.aria-pressed]="hideConf">
                            <mat-icon>{{ hideConf ? 'visibility_off' : 'visibility' }}</mat-icon>
                        </button>
                        <div *ngIf="form?.errors?.['mismatch'] && form?.touched" class="errorMesConfPass">Пароли не
                            совпадают
                        </div>
                    </mat-form-field>
                ...
                <section class="example-section">
                    ...
                    <mat-radio-group formControlName="gender">
                        <mat-radio-button class="example-margin" value="Мужской">Мужской</mat-radio-button>
                        <mat-radio-button class="example-margin" value="Женский">Женский</mat-radio-button>
                    </mat-radio-group>
                </section>

                <mat-form-field>
                    <mat-label class="type-font-form">Дата рождения</mat-label>
                    <input matInput [matDatepicker]="picker" placeholder="Дата рождения" formControlName="b_days"
                           type="b_days" required>
                    <mat-datepicker-toggle matSuffix [for]="picker"></mat-datepicker-toggle>
                    <mat-datepicker #picker></mat-datepicker>
                </mat-form-field>

                <span class="center margin-top">Вы уже зарегистрированы? <a
                        class="sing-up-link"
                        routerLink="registration"
                        (click)="goToLoginPage()">Login!</a></span>
                <mat-card-actions class="center margin-top">
                    <button mat-raised-button color="primary" type="submit" [disabled]="!form.valid">REGISTRATION
                    </button>
                </mat-card-actions>
            </form>
        </div>
...
export class RegistrationPagesComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.form = this.formBuilder.group({
                ...
                password: new FormControl('', [Validators.required, Validators.minLength(6)]), // User password
                conf_password: new FormControl('', [Validators.required, Validators.minLength(6)]), // User confirm password
                gender: [''], // User gender
    ...
    passwordMatchValidator(controlPasswordCONF: AbstractControl) {
        return controlPasswordCONF.get('password')?.value === controlPasswordCONF.get('conf_password')?.value
            ? null : {'mismatch': true};
    }
    ...
    submitRegistration(): void {
        if (this.form.valid) {
            ...
            const requestBody = {
                ...
                registration_data: this.currentDataRegistrationUser.registration_data.toString(),
            ...
            this.http.post('http://localhost:8001/api/user-temporary-data/addUserDataRegistration', requestBody).subscribe(
                response => {
